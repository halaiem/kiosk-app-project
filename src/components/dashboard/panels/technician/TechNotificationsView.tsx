...
interface TechNotificationsViewProps {
...
const FORWARD_ROLES: { key: UserRole; label: string; icon: string }[] = [
  ...
  { key: "mechanic", label: "Механик", icon: "Wrench" },
...
function generateDemoNotifications(): Notification[] {
  ...
  const titles = [
    ...
    "Ошибка CAN-шины: код P0420",
    "Давление масла ниже порога",
    "Превышение скорости на маршруте М12",
    "Износ тормозных колодок > 80%",
    "Отклонение от маршрута",
    "Просрочена диагностическая карта",
    "Уровень топлива критически низкий",
    "Обновление ПО бортового компьютера",
    "Плановое ТО через 500 км",
  ];
  const bodies = [
    "Зафиксировано превышение температуры охлаждающей жидкости до 105°C. Рекомендуется немедленная проверка системы охлаждения.",
    "Диагностический код P0420 — неэффективность каталитического нейтрализатора. Необходима проверка на СТО.",
    "Давление масла упало до 0.8 бар при норме 1.5-4.5 бар. Возможна утечка или износ масляного насоса.",
    "ТС №4521 превысило допустимую скорость на 25 км/ч на участке маршрута М12 (Ленинский пр.).",
    "Датчик износа тормозных колодок показывает остаточный ресурс менее 20%. Требуется замена.",
    "ТС №3301 отклонилось от утвержденного маршрута на 1.2 км. Связь с водителем установлена.",
    "Диагностическая карта ТС №2210 просрочена с 01.03.2026. Эксплуатация запрещена до прохождения ТО.",
    "Остаток топлива ТС №5102 составляет 8%. Рекомендуется направить на заправку.",
    "Доступна новая версия ПО v3.2.1 для бортового компьютера. Обновление включает исправления CAN-протокола.",
    "До планового ТО ТС №1155 осталось 500 км пробега. Запись на сервис обязательна.",
  ];
  const roles: (UserRole | "all")[] = ["technician", "all", "dispatcher", "mechanic", "admin"];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    result.push({
      id: `demo-notif-${String(i + 1).padStart(3, "0")}`,
