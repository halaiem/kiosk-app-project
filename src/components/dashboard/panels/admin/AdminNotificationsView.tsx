...
function generateDemoNotifications(): Notification[] {
  ...
  const titles = [
    ...
    "Сервер MRM-02 — нагрузка CPU превышает 85%",
    "Лицензия модуля диагностики истекает через 7 дней",
    "Отчёт: 3 транспортных средства без ОСАГО",
    "Обновление системы v4.2.1 доступно для установки",
    "Резервное копирование БД завершено успешно",
    "Нарушение SLA: время ответа API > 5 секунд",
    "Запрос на повышение прав пользователя Иванов А.С.",
    "Журнал аудита: 12 подозрительных входов за сутки",
    "Расписание ТО на следующую неделю не утверждено",
  ];
  const bodies = [
    "В системе зарегистрирован новый пользователь: Петров В.И., роль — диспетчер. Учётная запись ожидает подтверждения администратором. Проверьте данные и активируйте аккаунт.",
    "Сервер MRM-02 испытывает повышенную нагрузку: CPU 87%, RAM 72%. Возможно замедление обработки телеметрии. Рекомендуется проверить очередь задач и при необходимости перезапустить сервис.",
    "Срок действия лицензии модуля диагностики (OBD-Pro) истекает 17.04.2026. После истечения функции считывания кодов ошибок будут недоступны. Продлите лицензию заблаговременно.",
    "По результатам автоматической проверки выявлено 3 транспортных средства с истёкшим полисом ОСАГО: ТС №2201, №3345, №4102. Эксплуатация данных ТС запрещена до оформления страховки.",
    "Доступно обновление платформы до версии v4.2.1. Включены исправления безопасности, оптимизация отчётов и новый модуль уведомлений. Рекомендуется установить в ближайшее окно обслуживания.",
    "Резервное копирование базы данных выполнено успешно в 03:00. Размер архива: 4.7 ГБ. Контрольная сумма проверена. Копия сохранена на удалённое хранилище S3.",
    "Зафиксировано нарушение SLA: среднее время ответа API за последний час составляет 5.3 секунды при допустимом пороге 2 секунды. Необходимо провести анализ узких мест и масштабирование.",
    "Пользователь Иванов А.С. (механик) направил запрос на повышение прав доступа до уровня «Старший механик». Обоснование прилагается. Требуется рассмотрение и подтверждение.",
    "Система аудита зафиксировала 12 подозрительных попыток входа за последние 24 часа с 4 различных IP-адресов. Рекомендуется проверить журнал и при необходимости заблокировать адреса.",
    "Расписание технического обслуживания на следующую неделю (14.04–18.04) не утверждено ответственным лицом. В очереди 8 ТС. Утвердите расписание для запуска автоматических напоминаний.",
  ];
  const roles: (UserRole | "all")[] = ["admin", "all", "dispatcher", "technician", "mechanic"];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    result.push({
      id: `demo-admin-notif-${String(i + 1).padStart(3, "0")}`,
