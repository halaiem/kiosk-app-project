  ...
  return (
    <>
      ...
      >
        ...
        <div className={`flex items-baseline gap-1 transition-all duration-300 ${animating ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}>
          <span className="text-base tablet:text-xl font-black text-foreground tabular-nums leading-none">
            {current?.intervalMin ?? '—'}
          </span>
          <span className="text-[10px] tablet:text-xs font-semibold text-muted-foreground leading-none">мин</span>
        </div>
        <span className={`text-[9px] tablet:text-[11px] text-muted-foreground leading-none text-center transition-all duration-300 ${animating ? 'opacity-0' : 'opacity-100'}`}>
          {label}
        </span>
      </button>
