...
export default function EndShiftModal({ open, driver, dispatcherName, onClose, onConfirm }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-3xl mx-4 kiosk-surface rounded-3xl elevation-4 overflow-hidden animate-in zoom-in-95 duration-300" style={{ minHeight: '67vh' }}>
        <div className="flex h-full" style={{ minHeight: '67vh' }}>
          ...
          <div className="w-[65%] p-6 space-y-4 flex flex-col justify-center">
            ...
            <div className="flex gap-3">
              <button
                onClick={onClose}
                className="flex-1 py-4 rounded-2xl bg-muted text-foreground font-semibold text-xl transition-all active:scale-[0.97] ripple"
              >
                Отменить
              </button>
              <button
                onClick={() => onConfirm(rating)}
                className="flex-1 py-4 rounded-2xl bg-orange-500 hover:bg-orange-400 text-white font-semibold text-xl flex items-center justify-center gap-3 transition-all active:scale-[0.97] ripple elevation-2"
              >
