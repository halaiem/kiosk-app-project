...
export default function BreakModal({ isOpen, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="kiosk-surface rounded-3xl elevation-4 p-11 max-w-xl w-full mx-4 animate-scale-in">

        {/* Заголовок */}
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-warning/15 flex items-center justify-center">
              <Icon name="Coffee" size={32} className="text-warning-foreground" />
            </div>
            <div>
              <h2 className="font-bold text-foreground text-2xl">Перерыв</h2>
              <p className="text-muted-foreground text-base">Выберите длительность</p>
            </div>
          </div>
          {!active && (
            <button onClick={onClose} className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center ripple">
        ...
        {active ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-4 mb-8">
              {BREAK_OPTIONS.map(d => (
                <button key={d} onClick={() => setSelectedDuration(d)}
                  className={`py-5 rounded-2xl font-bold text-2xl transition-all ripple
            ...
            <button onClick={startBreak}
              className="w-full py-5 rounded-2xl bg-warning text-warning-foreground font-bold text-xl flex items-center justify-center gap-3 elevation-2 ripple active:scale-[0.98]">
