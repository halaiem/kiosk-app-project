...
export default function LoginPage({ onLogin, error, loading }: Props) {
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center kiosk-bg relative overflow-hidden">
      ...
      {/* Блок логотипов НАД формой авторизации */}
      <div className="animate-scale-in relative z-10 flex items-center justify-center gap-10 mb-10">
        {/* Логотип перевозчика */}
        {settings.carrierLogo && (
          <div className="flex flex-col items-center gap-3">
            <div className="w-40 h-40 rounded-3xl elevation-3 flex items-center justify-center overflow-hidden bg-white/10">
              <img src={settings.carrierLogo} alt={settings.carrierName} className="w-32 h-32 object-contain" />
            </div>
            <span className="text-3xl font-bold text-foreground tracking-tight">{settings.carrierName}</span>
          </div>
        )}

        {/* Разделитель — только если есть лого перевозчика */}
        {settings.carrierLogo && (
        ...
        {/* Логотип ИРИДА */}
        <div className="flex flex-col items-center gap-3">
          <div className="w-40 h-40 rounded-3xl elevation-3 flex items-center justify-center"
            style={{ backgroundColor: 'hsl(var(--kiosk-header-bg))' }}>
            <img src="https://cdn.poehali.dev/files/99eade92-26ae-4d2a-87f8-343f497fc065.png" alt="ИРИДА" className="w-28 h-28 object-contain" />
          </div>
      ...
      {/* Форма авторизации — увеличена в 2.5x */}
      <div className="animate-scale-in relative z-10 w-full max-w-2xl mx-4">
