"""API задач: CRUD, вложения S3, голосовые комментарии, архив, sidebar-config"""
import json
import os
import uuid
import base64
...
def handler(event, context):
    """Задачи: CRUD, вложения, голосовые комментарии, архив, sidebar-конфиг"""
    ...
    # Sidebar config actions don't require auth (read), write requires auth
    ...
    if action == 'upload_attachment' and method == 'POST':
        body = json.loads(event.get('body') or '{}')
        task_id = body.get('task_id')
        file_data = body.get('data', '')
        file_name = body.get('name', 'file')
        content_type = body.get('type', 'application/octet-stream')
        if not task_id or not file_data:
            cur.close(); conn.close()
            return resp(400, {'error': 'Укажите task_id и data'})
        try:
            raw = base64.b64decode(file_data)
            if len(raw) > MAX_FILE_BYTES:
                cur.close(); conn.close()
                return resp(400, {'error': f'Файл слишком большой (макс {MAX_FILE_BYTES // 1024 // 1024} МБ)'})
            ext = file_name.rsplit('.', 1)[-1].lower() if '.' in file_name else 'bin'
            key = f"tasks/{task_id}/attachments/{uuid.uuid4().hex}.{ext}"
            cdn_url = upload_to_s3(raw, key, content_type)
            cur.execute(
            ...
            att_id = cur.fetchone()['id']
            cur.close(); conn.close()
            return resp(201, {'id': att_id, 'url': cdn_url, 'name': file_name, 'size': len(raw), 'type': content_type})
    ...
    if action == 'add_comment' and method == 'POST':
        ...
        if file_data:
            try:
                ...
                file_size = len(raw)
                file_name = file_name or (f"voice_{uuid.uuid4().hex[:8]}.{ext}" if is_voice else 'file')

                # Transcribe voice with Yandex SpeechKit
                if is_voice:
                    ...
                    if speechkit_key:
                        try:
                            ...
                            req = urllib.request.Request(
                                ...
                                data=raw,
                                headers={
                                    'Authorization': f'Api-Key {speechkit_key}',
                            ...
                            with urllib.request.urlopen(req, timeout=15) as r:
                                ...
                                voice_transcript = result.get('result', '')
                        except Exception:
                            voice_transcript = None
        ...
        cur.execute(
            ...
            (task_id, user['id'], message or (voice_transcript or ''), file_url, file_name or None,
    ...
    if action == 'archive' and method == 'POST':
        ...
        cur.execute(f"UPDATE tasks SET is_archived = %s, updated_at = NOW() WHERE id IN ({placeholders})", [archive_val] + ids)
        cur.close(); conn.close()
